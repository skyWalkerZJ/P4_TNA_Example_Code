...
control basicSwitch(inout headers hdr,
...
)
{
    ...
    action l3_route(PortId_t port,bit<48> new_dst_mac)
    {
    ...
    action l2_rewrite_with_tunnel(PortId_t port,
                                        bit<14> tunnel_index,
                                        bit<5> tunnel_type)
    {
        ig_tm_md.ucast_egress_port=port;
    ...
    action l3_rewrite_with_tunnel(PortId_t port,
                                      bit<48> new_dst_mac,
                                      bit<14> tunnel_index,
                                      bit<5> tunnel_type)
    {
    ...
    table rewrite{
        ...
        actions={
            l2_forward;
            l2_rewrite_with_tunnel;
            l3_route;
            l3_rewrite_with_tunnel;
        }
        size=1024;
    }
...
control packet_capsulation(inout headers hdr,
    inout user_metadata user_md,
    in ingress_intrinsic_metadata_t ig_intr_md,
    in ingress_intrinsic_metadata_from_parser_t ig_prsr_md,
    inout ingress_intrinsic_metadata_for_deparser_t  ig_dprsr_md,
...
control dropByRate(inout headers hdr,
    inout user_metadata user_md,
    in ingress_intrinsic_metadata_t ig_intr_md,
    in ingress_intrinsic_metadata_from_parser_t ig_prsr_md,
    inout ingress_intrinsic_metadata_for_deparser_t  ig_dprsr_md,
    inout ingress_intrinsic_metadata_for_tm_t        ig_tm_md)
{
    ...
    {
        bit<10> r=rnd.get();
        user_md.packet_drop.drop_rate=r;
    }
    ...
    table setDropRate{
        ...
        default_action = setRate;
    }
    
    table packet_drop{
        key={
            user_md.packet_drop.drop_rate     : range;
        }
...
control packet_corruption(inout headers hdr,
    inout user_metadata user_md,
    in ingress_intrinsic_metadata_t ig_intr_md,
    in ingress_intrinsic_metadata_from_parser_t ig_prsr_md,
    inout ingress_intrinsic_metadata_for_deparser_t  ig_dprsr_md,
    inout ingress_intrinsic_metadata_for_tm_t        ig_tm_md)
{
    action vlan_vid_rewrite(bit<12> new_data)
    {
        hdr.vlan_tag[0].vid=new_data;
    }

    action ipv4_totalLen_rewrite(bit<16> new_data)
    {
        hdr.ipv4.totalLen=new_data;
    }

    action ipv4_ttl_rewrite(bit<8> new_data)
    {
        hdr.ipv4.ttl=new_data;
    }



    action ipv4_fragOffset_rewrite(bit<13> new_data)
    {
        hdr.ipv4.fragOffset=new_data;
    }

    action ipv4_ihl_rewrite(bit<4> new_data)
    {
        hdr.ipv4.ihl=new_data;
    }

    action ipv4_hdrChecksum_rewrite(bit<16> new_data)
    {
        hdr.ipv4.hdrChecksum=new_data;
    }

    action ipv4_protocol_rewrite(bit<8> new_data)
    {
        hdr.ipv4.protocol=new_data;
    }


    action ipv6_flowLabel_rewrite(bit<20> new_data)
    {
        hdr.ipv6.flowLabel=new_data;
    }

    action ipv6_payloadLen_rewrite(bit<16> new_data)
    {
        hdr.ipv6.payloadLen=new_data;
    }

    action ipv6_hopLimit_rewrite(bit<8> new_data)
    {
        hdr.ipv6.hopLimit=new_data;
    }

    action ipv6_nextHdr_rewrite(bit<8> new_data)
    {
        hdr.ipv6.nextHdr=new_data;
    }

    action tcp_seqNo_rewrite(bit<32> new_data)
    {
        hdr.tcp.seqNo=new_data;
    }

    action tcp_ackNo_rewrite(bit<32> new_data)
    {
        hdr.tcp.ackNo=new_data;
    }

    action tcp_flags_rewrite(bit<8> new_data)
    {
        hdr.tcp.flags=new_data;
    }

    action tcp_checksum_rewrite(bit<16> new_data)
    {
        hdr.tcp.checksum=new_data;
    }

    action tcp_window_rewrite(bit<16> new_data)
    {
        hdr.tcp.window=new_data;
    }

    action udp_checksum_rewrite(bit<16> new_data)
    {
        hdr.udp.checksum=new_data;
    }

    action udp_length_rewrite(bit<16> new_data)
    {
        hdr.udp.length=new_data;
    }
    ...
    table filed_update{
        ...
        actions={
            ...
            ipv4_totalLen_rewrite;
            ipv4_ttl_rewrite;
            ipv4_fragOffset_rewrite;
            ipv4_ihl_rewrite;
            ipv4_hdrChecksum_rewrite;
            ipv4_protocol_rewrite;

            ipv6_flowLabel_rewrite;
            ipv6_payloadLen_rewrite;
            ipv6_hopLimit_rewrite;
            ipv6_nextHdr_rewrite;
            
            tcp_seqNo_rewrite;
            tcp_ackNo_rewrite;
            tcp_flags_rewrite;
            tcp_checksum_rewrite;
            tcp_window_rewrite;
            
            udp_checksum_rewrite;
            udp_length_rewrite;
...
{
    apply{
        basicSwitch.apply(hdr,user_md,ig_intr_md,ig_prsr_md,ig_dprsr_md,ig_tm_md);
        dropByRate.apply(hdr,user_md,ig_intr_md,ig_prsr_md,ig_dprsr_md,ig_tm_md);
        packet_corruption.apply(hdr,user_md,ig_intr_md,ig_prsr_md,ig_dprsr_md,ig_tm_md);
        packet_capsulation.apply(hdr,user_md,ig_intr_md,ig_prsr_md,ig_dprsr_md,ig_tm_md);
    }
}
